...
async function getChangedFilesWithDiff() {
  ...
  function formatFileDiff(diff) {
    function formatHunk(hunk) {
      ...
      const regexForJustLinesAdded =
        /@@ \-([1-9][0-9]*) \+([1-9][0-9]*),([0-9]*) @@/g;
      const regexForJustLinesDeleted =
        /@@ \-([1-9][0-9]*),([1-9]*) \+([0-9]*),0 @@/g;
      const regexs = [
        {
          operation: "singleLineChange",
          regex: regexForSingleLineChange,
          handler: parseSingleLineChangesToLineNumbers,
        },
        {
          operation: "justLinesAdded",
          regex: regexForJustLinesAdded,
          handler: parseJustLinesAddedToLineNumbers,
        },
        {
          operation: "justLinesDeleted",
          regex: regexForJustLinesDeleted,
          handler: parseJustLinesDeletedToLineNumbers,
        },
      ];
    ...
    return {
      ...
      fromFile,
      toFile,
      hunks: hunks.slice(1).map(formatHunk),
    };
  }
