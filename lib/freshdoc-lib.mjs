...
let markdownFreshDocCodeBlocksRegex = /``` [a-zA-Z0-9]* @freshdoc .*/gi
...
export async function getItems() {
    ...
    for (const file of markdowns) {
        ...
        const lines = matchWithLineNumber(fileContents, markdownFreshDocCodeBlocksRegex)
        for (const line of lines) {
            const [language, freshDocReference, declaration] = line.line.slice(3).trim().split(" ")
            const [referencedFile, codeBlockRange] = declaration.split(":")
            const [codeBlockRangeStart, codeBlockRangeEnd] = codeBlockRange.split("-").map((line) => parseInt(line))
            codeBlocks.push({
                ...
                referencedCodeFilename: path.resolve(path.dirname(file.history[0]) + "/" + referencedFile),
                codeBlockRangeStart,
                codeBlockRangeEnd
            })
        }
